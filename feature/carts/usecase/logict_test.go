...
func TestCreateData(t *testing.T) {
	...
	productPrice := 2000
	cekCurrentQty := 1000
	// mockdata3 := domain.Cart{ProductID: 9, Qty: 1000, UserID: 1}
	mockData := domain.Cart{ID: 1, ProductID: 9, Qty: 1000, Subtotal: 10900, Grandtotal: 20000, UserID: 2}
	...
	t.Run("Insufficient Stock", func(t *testing.T) {
		repo.On("GetStockProduct", mock.Anything).Return(1, nil).Once()
		useCase := New(repo)
		res, err := useCase.CreateData(mockData)
		if mockData.Qty > 1000 {
	...
	t.Run("succes2", func(t *testing.T) {
		useCase := New(repo)
		res, err := useCase.CreateData(mockData)
		if mockData.Qty == 9 || mockData.ProductID == 10 {
			...
			repo.On("GetStockProduct", mock.Anything).Return(productStock).Once()
			repo.On("GetPriceProduct", mock.Anything).Return(productPrice).Once()
			repo.On("CheckCart", mock.Anything, mock.Anything).Return(false, 1, 1).Once()
			repo.On("InsertData", mock.Anything).Return(1, 1).Once()
...
func TestUpdateData(t *testing.T) {
	...
	t.Run("Success Update product", func(t *testing.T) {
		repo.On("GetPriceProduct", mock.Anything).Return(1000, nil).Once()
	...
	t.Run("Failed Update product", func(t *testing.T) {
		repo.On("GetPriceProduct", mock.Anything).Return(1000, errors.New("error")).Once()
