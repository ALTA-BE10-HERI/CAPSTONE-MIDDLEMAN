...
func (oc *orderUseCase) GetDetail(orderName string) (grandTotal, idOrder int, status string, err error) {
	grandTotal, idOrder, status, err = oc.orderData.GetDetailData(orderName)
	if grandTotal == 0 {
	...
	return grandTotal, idOrder, status, nil
}
